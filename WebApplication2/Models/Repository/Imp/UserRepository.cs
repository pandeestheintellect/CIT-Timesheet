...
namespace Eng360Web.Models.Repository.Interface
{
    public class UserRepository : IUserRepository
    {
        ...
        {
            var user = Eng360DB.eng_users.Where(a => a.UserName == username.Trim() && a.Password == password.Trim() && a.IsActive==1).FirstOrDefault();
        ...
        {
            var edituser = Eng360DB.eng_users.Find(user.UserID);
            ...
            Eng360DB.Entry(edituser).State = EntityState.Modified;
        ...
        public bool chkUser(string username)
        {
            try
            {
                var user = Eng360DB.eng_users.Where(a => a.UserName == username && a.IsActive ==1).SingleOrDefault();
                if (user == null)
                    return false;
                else
                    return true;
